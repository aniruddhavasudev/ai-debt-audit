...
- **Giant-dump commit thresholds** — `GIANT_DUMP_MIN_FILES` (15) and `GIANT_DUMP_MIN_LINES` (500) in `scripts/git-mine.js` are a starting guess, not derived from real repo data. Testing against real "AI dumped 2,000 lines in one commit" cases (and real, legitimately large-but-fine commits — a big deletion, a generated-file update, a vendored dependency bump) to see whether these thresholds actually separate the two would be valuable.
