...
function applyConfigFilters(results, config, { ruleIdKey, pathKey }) {
...
function scoreDependencyVulnerabilities(pipAuditResults, npmAuditResults) {
...
function renderShieldsBadge(composite, tier) {
  return {
    schemaVersion: 1,
    label: "ai-debt score",
...
function renderHtml({ composite, tier, technical, duplication, historicalSecrets, bandit, dependencyVulns, cognitive, intent, top, repoPath, weights }) {
...
function main() {
  ...
  // repoRoot/toRelative are defined earlier now (config filtering needs
  // them too) — just applying them here to the remaining findings.
...
// Exported for scripts/score.test.js — this is the whole scoring engine's
// pure-function surface, deliberately separate from main()'s I/O/CLI
// concerns so it can be unit-tested without touching the filesystem.
export {
  globToRegExp,
  loadConfig,
  applyConfigFilters,
